...
messaging.upIPFS(
  ...
  path.join(path.resolve(),`/data/messaging/${process.env.DATA_SUBPATH || '/d1/'}/DataBase/`)
)
...
export default class DApp {
  constructor (params) {
    ...
    if (params.contract && process.env.DC_NETWORK !== 'local') {
      ...
      console.log(_config)
      const contract = Utils.LocalGameContract(_config.network.contracts.paychannelContract)
