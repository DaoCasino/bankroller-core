  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (allowance < amount) {
      ...
      const receipt = await ERC20.methods.approve(
        ...
        gas      : (await ERC20.methods.approve(spender, parseInt(amount)).estimateGas({from : Eth.acc.address}))
      }).on('error', err => {
...
export default class DApp {
  ...
  async _openChannel (params) {
    ...
    await ERC20approve(this.PayChannel().options.address, bankroller_deposit * 100)
