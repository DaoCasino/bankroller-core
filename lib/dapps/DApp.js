...
export default class DApp {
  ...
  async _call (data) {
    ...
    // Подписываем состояние канала
    if (!user.paychannel.State) { user.paychannel.State = new channelState() }
    ...
    user.paychannel.State.set( Object.assign(
      state_data,
      {'_sign' : state_sign }
    ) )
    ...
    this.response(data, {
      ...
      state    : user.paychannel.State.get(),
      returns  : returns
    }, user.room)
  }
  ...
  _updateState (data) {
    ...
    const last_state = user.paychannel.State.get()
    
    // Проверяем содержимое
    for (let k in last_state) {
      if (k == '_sign') continue
      if (last_state[k] !== data.state[k]) {
        console.error('user channel state != last bankroller state', last_state, data.state)
        console.error(last_state[k] + '!==' + data.state[k])
    ...
    const recover_openkey = web3.eth.accounts.recover(state_hash, data.state._sign)
    if (recover_openkey.toLowerCase() !== data.user_id.toLowerCase()) {
      console.error('State ' + recover_openkey + '!=' + data.user_id)
      this.response(data, {status:'error', error:'invalid sig'}, user.room)
      return
    }

    user.paychannel.State.set( data.state )
