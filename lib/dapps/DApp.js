...
const ERC20approve = async function(spender, amount, callback = false) {
  return new Promise(async (resolve, reject) => {
    let allowance = await ERC20.methods
      .allowance(Eth.acc.address, spender)
      .call()
      .catch(err => {
        reject(err);
      });
  ...
  return {
    ...
    addPlayerSigned(state_data) {
      ...
      if (!Eth.checkHashSig(state_hash, state_data._sign, player_openkey)) {
        console.error("Invalid state sign");
        console.error(state_hash);
        return false;
      }
...
export default class DApp {
  constructor(params) {
    ...
    this.users = {};
    this.sharedRoom = new messaging.RTC(
      Eth.acc.address || false,
      "dapp_room_" + this.hash
    );
    this.timer = 10;
    this.checkTimeout = 0;
    ...
    this.PayChannel = new web3.eth.Contract(
      this.contract_abi,
  ...
  async _checkOpenChannel(data) {
    ...
    if (
      ...
      this.response(
        data,
        { error: "channel not found", info: channel },
        response_room
      );
    }
  }
