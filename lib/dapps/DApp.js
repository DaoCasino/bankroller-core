...
const ERC20approve = async function (spender, amount, callback = false) {
  return new Promise(async (resolve, reject) => {
    let allowance = await ERC20.methods.allowance(Eth.acc.address, spender)
      .call()
    ...
    if (allowance < amount || (amount === 0 && allowance !== 0)) {
      const receipt = await ERC20.methods.approve(
        ...
        .catch(err => reject(new Error(false, err)))
...
const channelState = function (player_openkey = false) {
  ...
  return {
    ...
    addPlayerSigned (state_data) {
      ...
      if (!Eth.checkHashSig(state_hash, state_data._sign, player_openkey)) {
        console.error('State invalid with sign', player_openkey)
        return false
      }
...
export default class DApp {
  constructor (params) {
    ...
    this.users        = {}
    this.sharedRoom   = new messaging.RTC((Eth.acc.address || false), _config.dappRoom + this.hash)
    this.timer        = 10
    this.checkTimeout = 0
  ...
  async _checkOpenChannel (data) {
    ...
    let channel
    try {
      channel = await this.PayChannel.methods.channels(l_channel.channel_id).call()
    } catch (e) {
      console.error('check channel .call error', e)
      this.response(data, { error:'channel not found', info: {channel: channel, l_channel: l_channel, data: data}}, response_room)
      return
    }
  ...
  async _call (data) {
    ...
    const hash = Utils.sha3(...to_verify_hash)
    // const recover_openkey = web3.eth.accounts.recover(Utils.sha3(...to_verify_hash), data.sign)
    if (!Eth.checkHashSig(hash, data.sign, data.user_id)) {
