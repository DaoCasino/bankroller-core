...
const ERC20approve = async function (spender, amount, callback = false) {
  return new Promise(async (resolve, reject) => {
    let [err, allowance] = await to(ERC20.methods.allowance(Eth.acc.address, spender).call())
    if (err) reject(err)
...
export default class DApp {
  constructor (params) {
    ...
    this.approveGameContract( this.PayChannel.options.address , 100000000, () => {    
  ...
  async _call (data) {
    ...
    if (last_state && last_state._playerBalance) {
      user_bets = user.paychannel.info.playerBalance
    }

    if ( user_bets * 1 < data.data.user_bet * 1 ) {
