...
const ERC20approve = async function (spender, amount, callback = false) {
  return new Promise(async (resolve, reject) => {
    Utils.debugLog('Check how many tokens user ' + Eth.acc.address + ' is still allowed to withdraw from contract ' + spender + ' . . . ', {name:_config.loglevel, flag:true})
...
export default class DApp {
  constructor (params) {
    ...
    if (params.contract) {
      ...
      Utils.debugLog('Standart payChannel contract is add', _config.loglevel)
