...
export default class DApp {
  ...
  async _newUser (params) {
    ...
    await this.bankrollRSA.generateRSAkey()
    ...
    const listen_all = async data => {
      ...
      if (data.action === 'call') {
        ...
        const returns  = User.logic[data.func.name].apply(this, args.new_args)
    ...
    setTimeout(async () => {
      if (connection_id && this.bankrollRSA.RSA.n.toString(16)) {
        this.response(params, {
          n  : this.bankrollRSA.RSA.n.toString(16),
          id : connection_id
        }, this.sharedRoom)
