  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (allowance < amount) {
      ...
      const receipt = await ERC20.methods.approve(
        ...
        gas      : (await ERC20.methods.approve(spender, parseInt(amount * 20)).estimateGas({from : Eth.acc.address}))
      }).on('error', err => {
...
export default class DApp {
  constructor (params) {
    ...
    console.log('Contract',this.contract_address)
  ...
  _newUser (params) {
    ...
    setTimeout(() => {
      ...
      if (typeof this.bankrollRSA.RSA.n === 'undefined' ||
      typeof this.bankrollRSA.RSA.e === 'undefined') {
        const error = new Error('RSA Key not generation')
        this.response(params, {error: error}, this.sharedRoom)
        return error
      }

      this.response(params, {
        id : connection_id,
        n  : this.bankrollRSA.RSA.n.toString(16)
      }, this.sharedRoom)
      console.log(this.bankrollRSA.RSA.n.toString(16))
  ...
  async _openChannel (params) {
    ...
    const _E                 = params.open_args.RSA_e
    const _N                 = params.open_args.RSA_n
    ...
    await ERC20approve(this.PayChannel().options.address, bankroller_deposit)
  ...
  async _closeChannel (params) {
    ...
    console.log('DANUNAHUY', signed_args, hash)
  ...
  _updateState (params, callback = false) {
    ...
    if (!DCLib.checkHashSig(Utils.sha3(channel_id, player_balance, bankroller_balance, session, bool), signed_args, player_address)) {
