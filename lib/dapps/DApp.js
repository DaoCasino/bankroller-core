  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (allowance < amount || (amount === 0 && allowance !== 0)) {
      ...
      if (!['0x01', '0x1', true].includes(receipt.status)) {
        reject(new Error(receipt, receipt.err))
        return
      }
    }
...
export default class DApp {
  ...
  async approveGameContract (address, amount = 100000000) {
    ...
    console.log('ERC20approve complete')
  }
  ...
  async _call (data) {
    ...
    if (user.paychannel.State.hasUnconfirmed()) {
      this.response(data, { error: 'Player ' + data.user_id + ' not confirm previous channel state' }, user.room)
    ...
    if (recover_openkey.toLowerCase() !== data.user_id.toLowerCase()) {
      this.response(data, { error: 'Invalid sig!' }, user.room)
    ...
    const confirmRandom = function (data) {
      ...
      try {
        ...
        this.response(data, { error: 'Cant get sha3 from rnd_hash_args' }, user.room)
      ...
      try {
        ...
        this.response(data, { error: 'Cant RSA.signHash from rnd_hash' }, user.room)
    ...
    try {
      ...
      this.response(data, { error: 'Cant call gamelogic function ' + data.func.name }, user.room)
    ...
    if (!user.paychannel.State.addBankrollerSigned( state_data )) {
      this.response(data, { error: 'Prodblem with save last channel state - addBankrollerSignedState' }, user.room)
