...
export default new class Eth {
  ...
  async initAccount (callback = false) {
    ...
    this.acc = this.web3.eth.accounts.privateKeyToAccount( privateKey )
    web3.eth.accounts.wallet.add( privateKey )

    const AccountETHBalance = await this.getEthBalance(this.acc.address)
    if (AccountETHBalance) {
      Utils.debugLog(`Account ${this.acc.address} ETH balance: ${AccountETHBalance}`, _config.loglevel)
    }
