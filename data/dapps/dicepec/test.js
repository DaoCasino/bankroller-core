/* eslint-env mocha */
/* global DCLib describe it chai sinon localStorage */
...
function createDApp () {
  App = new DCLib.DApp({slug: 'dicetest_v12'})
  window.App = App
}
...
describe('Test dicegame', function () {
  ...
  describe('Create ecosystem', function () {
    it('Init account', function () {
      // Arrange
      // const default_address = '0x520B159808CEb56d5780Dd0CeaFC9FC9Fa60d605'
      const privateKey      = '0xfe9db3e341f5f0e9414fdf0d39719a44ad994cc6cc9e74ea6c7ee6b205eb602e'
        
      // Act
      DCLib.Account.initAccount()


      if (DCLib.Account.get().openkey) {
        _openkey = DCLib.Account.get().openkey
      } else {

        localStorage.web3wallet = JSON.stringify(
          DCLib.Account.web3.eth.accounts.encrypt(
            privateKey,
            '1234'
          )
        )

        DCLib.Account.web3.eth.accounts.wallet.add(privateKey)
        DCLib.Account.unlockAccount()
        _openkey = DCLib.Account.get().openkey
        console.log('openkey', _openkey)
      }
