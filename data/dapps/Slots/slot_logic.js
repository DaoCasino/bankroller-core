...
DCLib.defineDAppLogic("slot_game_E", function() {
  ...
  const S_BONUS = [
    [[3, 2, 2, 4, 4], [12, 15, null, null, null], [20, 23, 25, null, null]],
    [[1, 2, 2, 2, 4], [9, 11, 16, null, null], [20, 17, null, null, null]]
  ]
  ...
  class Slot {
    ...
    _getPaytableEntry(symbols, line) {
      ...
      if (!first) first = S_SYMBOLS.Wild

      if (first === S_SYMBOLS.Bonus) return null
    ...
    _getBaseBonusMultiplier(symbols) {
      ...
      for (let col = 0; col < S_COLS; col++) {
        for (let row = 0; row < S_ROWS; row++) {
          if (symbols[col][row] === S_SYMBOLS.Bonus) ++count
        }
      }
    ...
    _getFinalBonusMultiplier(bonus) {
      ...
      for (let i = 0; i < bonus.length; i++) result += bonus[i]
    ...
    spin(bet, randoms) {
      ...
      for (let line = 0; line < this.lines.length; line++) {
        ...
        if (entry) {
          let profit = bet * S_PAYTABLE[entry.symbol][S_COLS - entry.count]
          win += profit
          winLines.push([line, entry.count])
          console.log("line: ", line, entry, profit)
        }
      }
      ...
      if (bonusBaseMultiplier) {
        const scenarioNumber = randoms[5]
  ...
  return {
    play: function(userBet, gameData, randoms) {
      ...
      // const Game = function(bet, totalBet, rnd_hash){
      const creditprice = userBet / totalBet
