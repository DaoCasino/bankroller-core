...
DCLib.defineDAppLogic('slot_game_a', function (payChannel) {
  const S_ROWS = 3
  const S_COLS = 5
  ...
  class Slot {
    ...
    spin (bet, randoms) {
      ...
      for (let col = 0; col < S_COLS; col++) {
        ...
        let reel = []
        for (let row = 0; row < S_ROWS; row++) { reel.push(reels[(offset + row) % reels.length][col]) }
        symbols.push(reel)
      }
  ...
  const Game = function (bet, totalBet, rnd_hash) {
    const creditprice = bet / totalBet
    ...
    let res = slotGame.spin(totalBet / S_LINES.length, randoms)
