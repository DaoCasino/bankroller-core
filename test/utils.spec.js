...
describe('Test Utils bankroller', () => {
  ...
  test('checksum', () => {
    ...
    expect(checksum).toBe('0xd34128b373fda1504ae3e9358b6c4d20a115354bfd086ffa25772bb7c934fcde')
  })
  ...
  test('hextoString', () => {
    expect(typeof Utils.hexToString(web3_utils.utf8ToHex('Hello hex'))).toBe('string')
    expect(Utils.hexToString(web3_utils.utf8ToHex('Hello hex')).indexOf('Hello hex') !== -1).toBe(true)
  })
  ...
  test('Reverse For It', () => {
    const obj_one = {a: 1, b: 2}
    const obj_two = {}

    Utils.reverseForIn(obj_one, i => {
      obj_two[i] = obj_one[i] 
    })
    
    expect(obj_two).toHaveProperty('a')
    expect(obj_two).toHaveProperty('b')
    expect(obj_two).toEqual({b: 2, a: 1})
  })
